...
## Phase 6.1 - Post-P6 Fix Queue (From Skill Health Report)
Goal: stabilize archetype coverage and eliminate loop-risk/spam-inflated policy behavior in generic heuristic runs.

Data source:
- `docs/UPA_SKILL_HEALTH_2026-02-09.json` (`20` seeds, `40` turns, heuristic policy)

### P6.1.PR1 - Hunter crash unblock (hard blocker)
- Scope:
  - [x] Fix `HUNTER` harness runtime crash: `Cannot read properties of undefined (reading 'position')`.
  - [x] Guard displacement/path processing when target actor is removed mid-effect chain (`packages/engine/src/systems/effect-engine.ts`).
  - [x] Add deterministic regression test for previously failing seed (`upa-seed-13`) in `packages/engine/src/__tests__/balance_harness.test.ts`.
- Acceptance tests:
  - [x] `npx tsx packages/engine/scripts/runUpaOutlierAnalysis.ts 80 60 HUNTER` completes without runtime exception.
  - [x] Hunter appears in skill health report without `failures[]` entry.

### P6.1.PR2 - Loop-risk suppression (Shield Throw / Shield Bash)
- Scope:
  - [x] Penalize repeated no-progress skill actions in generic one-ply selector.
  - [x] Filter non-progressing damage-target candidates and avoid empty damage casts.
  - [x] Tune utility so objective/mobility beats low-yield spam for Vanguard/Skirmisher.
- Acceptance tests:
  - [x] `SHIELD_THROW` no longer classified `loop-risk` in regenerated skill health report.
  - [x] `SHIELD_BASH` no longer classified `spam-inflated` in regenerated skill health report.

### P6.1.PR3 - Firemage timeout collapse recovery
- Scope:
  - [x] Reduce Firemage timeout trap (timeout collapsed from ~`0.96` to `0` on 80-seed report run).
  - [x] Improve objective conversion while keeping deterministic policy.
  - [x] Add anti-spam penalties so `FIREWALK` usage is conditional on measurable progress.
- Acceptance tests:
  - [x] Firemage heuristic timeout rate reduced on 80-seed sample versus baseline.
  - [x] Firemage `avgFloor` improved while hazard breach rate remains `0`.

### P6.1.PR4 - Policy-blocked starting-skill coverage
- Scope:
  - [x] Ensure starting loadout skills are reachable by generic policy where legal (`ABSORB_FIRE`, `VAULT`, `WITHDRAWAL`, Hunter kit).
  - [x] Add per-archetype minimum kit-usage assertions in harness tests.
- Acceptance tests:
  - [x] Policy-blocked set reduced to non-cast baseline skills (`AUTO_ATTACK`, `BASIC_MOVE`) in health report.
  - [x] New tests fail if representative starter skills remain permanently uncast.

### P6.1.PR5 - Health report operationalization
- Scope:
  - [x] Wire `runSkillHealthReport.ts` into npm workflow (`npm run upa:health`).
  - [x] Add threshold-based fail/warn policy for `loop-risk` and archetype runtime failures (`maxLoopRisk`, `maxFailures` args).
- Acceptance tests:
  - [x] One command produces updated health report and threshold summary.
  - [x] CI/nightly output highlights regressions with machine-readable labels.

### Phase 6.1 Exit Criteria
- [x] No archetype runtime failures in health report (`failures[]` empty).
- [x] No high-frequency loop-risk tags on core player skills.
- [x] Firemage and Hunter produce stable, non-pathological telemetry profiles.
- [x] Skill health labels are included in regular balancing cadence.

## Phase 6.2 - Calibration and Expansion
Goal: move from stability fixes to true balance calibration across all archetypes and full skill coverage.

### P6.2.PR1 - Full-sample calibration pass
- Scope:
...
- Scope:
  - [x] Normalize dynamic numeric grades by role context so movement/control skills are not falsely undervalued.
  - [x] Add explicit role-weight presets (`damage`, `control`, `mobility`, `sustain`) for analysis-only grading.
- Acceptance tests:
  - [x] Dynamic artifact regenerated with role-adjusted fields (`numericGradeRaw`, `rolePreset`, `roleAdjustedGrade`) in `docs/UPA_SKILL_GRADES_DYNAMIC.json`.
  - [x] Drift report remains deterministic with same inputs (`docs/UPA_SKILL_GRADE_DRIFT.json` regenerated).

### P6.2.PR3 - Skill coverage completion
- Scope:
  - [x] Keep `no-data` limited to non-player/test-only/boss internals; enforce player-facing floor via health thresholds.
  - [x] Add/retain harness coverage checks for representative starter skills and policy-blocked baseline exceptions only.
- Acceptance tests:
  - [x] Player-facing no-data gate added in health report (`maxPlayerFacingNoData`) and checked in CI script.
  - [x] Current report limits policy-blocked to baseline non-cast skills: `AUTO_ATTACK`, `BASIC_MOVE`.

### P6.2.PR4 - Matchup matrix and balancing loop
- Scope:
  - [x] Run PvP UPA matrix across archetype pairs (`heuristic vs heuristic`) and commit artifact.
  - [x] Identify dominant pairings and publish top-3 tuning candidates.
- Acceptance tests:
  - [x] Matchup matrix artifact committed to `docs/UPA_PVP_MATCHUP_MATRIX.json`.
  - [x] Top 3 imbalance candidates captured from matrix output and tracked below.

### P6.2.PR5 - Release gate hardening
- Scope:
  - [x] Add CI threshold checks for hard regressions (runtime failures, loop-risk spikes, player-facing no-data).
...
- [x] Player-facing skill set has near-complete telemetry coverage (policy-blocked limited to baseline non-cast movement/auto skills).
- [x] Matchup matrix exists and drives prioritized balance work.
...
- Calibration (`docs/UPA_CALIBRATION_BASELINE.json`, `300` seeds, `80` turns, heuristic):
  ...
  - `NECROMANCER`: `winRate=0`, `timeout=0.0467`, `avgFloor=2.7667`, `hazard=0.8967`, `UPA=0.2257`
  - `HUNTER`: `winRate=0`, `timeout=0.0233`, `avgFloor=2.4033`, `hazard=0.8167`, `UPA=0.2259`
  - `ASSASSIN`: `winRate=0`, `timeout=0.07`, `avgFloor=2.7333`, `hazard=0.5733`, `UPA=0.2257`
- Matchup matrix (`docs/UPA_PVP_MATCHUP_MATRIX.json`, `30` seeds, `60` turns):
  - Top imbalance candidates:
    - `FIREMAGE vs VANGUARD` (`dominanceDelta=1.0`)
    - `FIREMAGE vs SKIRMISHER` (`dominanceDelta=1.0`)
    - `FIREMAGE vs NECROMANCER` (`dominanceDelta=1.0`)
