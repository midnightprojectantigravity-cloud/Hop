/**
 * AUTO-GENERATED FILE. DO NOT EDIT.
 * Generated by: packages/engine/scripts/generateSkillRegistry.ts
 * Skills discovered: 45 (ABSORB_FIRE, ARCHER_SHOT, AUTO_ATTACK, BASIC_ATTACK, BASIC_AWARENESS, BASIC_MOVE, BLIND_FIGHTING, BOMB_TOSS, BULWARK_CHARGE, COMBAT_ANALYSIS, CORPSE_EXPLOSION, DASH, FALCON_APEX_STRIKE, FALCON_AUTO_ROOST, FALCON_COMMAND, FALCON_HEAL, FALCON_PECK, FALCON_SCOUT, FIREBALL, FIREWALK, FIREWALL, GRAPPLE_HOOK, JUMP, KINETIC_TRI_TRAP, MULTI_SHOOT, ORACLE_SIGHT, RAISE_DEAD, SENTINEL_BLAST, SENTINEL_TELEGRAPH, SET_TRAP, SHADOW_STEP, SHIELD_BASH, SHIELD_THROW, SMOKE_SCREEN, SNEAK_ATTACK, SOUL_SWAP, SPEAR_THROW, STANDARD_VISION, SWIFT_ROLL, TACTICAL_INSIGHT, THEME_HAZARDS, TIME_BOMB, VAULT, VIBRATION_SENSE, WITHDRAWAL)
 */
import type { SkillDefinition } from '../types';
import { ABSORB_FIRE } from '../skills/absorb_fire';
import { ARCHER_SHOT } from '../skills/archer_shot';
import { AUTO_ATTACK } from '../skills/auto_attack';
import { BASIC_ATTACK } from '../skills/basic_attack';
import { BASIC_AWARENESS } from '../skills/basic_awareness';
import { BASIC_MOVE } from '../skills/basic_move';
import { BLIND_FIGHTING } from '../skills/blind_fighting';
import { BOMB_TOSS } from '../skills/bomb_toss';
import { BULWARK_CHARGE } from '../skills/bulwark_charge';
import { COMBAT_ANALYSIS } from '../skills/combat_analysis';
import { CORPSE_EXPLOSION } from '../skills/corpse_explosion';
import { DASH } from '../skills/dash';
import { FALCON_APEX_STRIKE } from '../skills/falcon_apex_strike';
import { FALCON_AUTO_ROOST } from '../skills/falcon_auto_roost';
import { FALCON_COMMAND } from '../skills/falcon_command';
import { FALCON_HEAL } from '../skills/falcon_heal';
import { FALCON_PECK } from '../skills/falcon_peck';
import { FALCON_SCOUT } from '../skills/falcon_scout';
import { FIREBALL } from '../skills/fireball';
import { FIREWALK } from '../skills/firewalk';
import { FIREWALL } from '../skills/firewall';
import { GRAPPLE_HOOK } from '../skills/grapple_hook';
import { JUMP } from '../skills/jump';
import { KINETIC_TRI_TRAP } from '../skills/kinetic_tri_trap';
import { MULTI_SHOOT } from '../skills/multi_shoot';
import { ORACLE_SIGHT } from '../skills/oracle_sight';
import { RAISE_DEAD } from '../skills/raise_dead';
import { SENTINEL_BLAST } from '../skills/sentinel_blast';
import { SENTINEL_TELEGRAPH } from '../skills/sentinel_telegraph';
import { SET_TRAP } from '../skills/set_trap';
import { SHADOW_STEP } from '../skills/shadow_step';
import { SHIELD_BASH } from '../skills/shield_bash';
import { SHIELD_THROW } from '../skills/shield_throw';
import { SMOKE_SCREEN } from '../skills/smoke_screen';
import { SNEAK_ATTACK } from '../skills/sneak_attack';
import { SOUL_SWAP } from '../skills/soul_swap';
import { SPEAR_THROW } from '../skills/spear_throw';
import { STANDARD_VISION } from '../skills/standard_vision';
import { SWIFT_ROLL } from '../skills/swift_roll';
import { TACTICAL_INSIGHT } from '../skills/tactical_insight';
import { THEME_HAZARDS } from '../skills/theme_hazard';
import { TIME_BOMB } from '../skills/time_bomb';
import { VAULT } from '../skills/vault';
import { VIBRATION_SENSE } from '../skills/vibration_sense';
import { WITHDRAWAL } from '../skills/withdrawal';

export const GENERATED_COMPOSITIONAL_SKILLS = {
    ABSORB_FIRE,
    ARCHER_SHOT,
    AUTO_ATTACK,
    BASIC_ATTACK,
    BASIC_AWARENESS,
    BASIC_MOVE,
    BLIND_FIGHTING,
    BOMB_TOSS,
    BULWARK_CHARGE,
    COMBAT_ANALYSIS,
    CORPSE_EXPLOSION,
    DASH,
    FALCON_APEX_STRIKE,
    FALCON_AUTO_ROOST,
    FALCON_COMMAND,
    FALCON_HEAL,
    FALCON_PECK,
    FALCON_SCOUT,
    FIREBALL,
    FIREWALK,
    FIREWALL,
    GRAPPLE_HOOK,
    JUMP,
    KINETIC_TRI_TRAP,
    MULTI_SHOOT,
    ORACLE_SIGHT,
    RAISE_DEAD,
    SENTINEL_BLAST,
    SENTINEL_TELEGRAPH,
    SET_TRAP,
    SHADOW_STEP,
    SHIELD_BASH,
    SHIELD_THROW,
    SMOKE_SCREEN,
    SNEAK_ATTACK,
    SOUL_SWAP,
    SPEAR_THROW,
    STANDARD_VISION,
    SWIFT_ROLL,
    TACTICAL_INSIGHT,
    THEME_HAZARDS,
    TIME_BOMB,
    VAULT,
    VIBRATION_SENSE,
    WITHDRAWAL,
} satisfies Record<string, SkillDefinition>;
