...
export const resolveAcaeRuleset = (state: GameState): NonNullable<GameState['ruleset']> => {
    ...
    const defaultCapabilityLoadoutPassivesEnabled =
        typeof process !== 'undefined' && process.env?.HOP_CAPABILITY_LOADOUT_PASSIVES === '1';
    return {
        ...(state.ruleset || {}),
        ailments: {
        ...
        capabilities: {
            loadoutPassivesEnabled:
                state.ruleset?.capabilities?.loadoutPassivesEnabled ?? defaultCapabilityLoadoutPassivesEnabled,
