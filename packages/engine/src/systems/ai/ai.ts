...
type EnemyActionResult = { entity: Entity; nextState: GameState; message?: string };
export type EnemyAiHandler = (enemy: Entity, playerPos: Point, state: GameState & { occupiedCurrentTurn?: Point[] }) => EnemyActionResult;

const computeRaiderAction: EnemyAiHandler = (enemy, playerPos, state) => {
    const dist = hexDistance(enemy.position, playerPos);
...
const computePouncerAction: EnemyAiHandler = (enemy, playerPos, state) => {
    const dist = hexDistance(enemy.position, playerPos);
    const isInLine = (enemy.position.q === playerPos.q) || (enemy.position.r === playerPos.r) || (enemy.position.s === playerPos.s);
    if (isInLine && dist >= 2 && dist <= 4) {
        return {
...
const computeArcherAction: EnemyAiHandler = (enemy, playerPos, state) => {
    const dist = hexDistance(enemy.position, playerPos);
...
const computeBomberAction: EnemyAiHandler = (enemy, playerPos, state) => {
    const dist = hexDistance(enemy.position, playerPos);
...
const computeDefaultEnemyAction: EnemyAiHandler = (enemy, playerPos, state) => {
    const dist = hexDistance(enemy.position, playerPos);
...
const ENEMY_SUBTYPE_AI_HANDLERS: Record<string, EnemyAiHandler> = {
...
export const registerEnemySubtypeAiHandler = (subtype: string, handler: EnemyAiHandler) => {
    ENEMY_SUBTYPE_AI_HANDLERS[subtype] = handler;
};
...
export const computeEnemyAction = (bt: Entity, playerMovedTo: Point, state: GameState & { occupiedCurrentTurn?: Point[] }): EnemyActionResult => {
    ...
    const handler = bt.subtype ? ENEMY_SUBTYPE_AI_HANDLERS[bt.subtype] : undefined;
    if (handler) {
        return handler(bt, playerMovedTo, state);
    }

    return computeDefaultEnemyAction(bt, playerMovedTo, state);
};
