...
import { type DynamicSkillMetric, type SkillTelemetryTotals } from './skill-grading';
import {
    getStrategicPolicyProfile,
    type StrategicIntent,
    type StrategicPolicyProfile
} from '../ai/strategic-policy';
import { compareRuns } from './balance-harness-summary';
import {
    chooseFromSeeded,
    incrementHistogram,
...
export { summarizeBatch, summarizeMatchup } from './balance-harness-summary';
...
export const selectByOnePlySimulation = (
    ...
    return chooseFromSeeded(ties, `${simSeed}:sim`, decisionCounter);
};
...
const selectAction = (
    ...
    if (policy === 'random') {
        ...
        return { action: chooseFromSeeded(options, simSeed, decisionCounter), strategicIntent };
    }
