      ...
      if (t) {
        ...
        if (t.type === 'enemy') {
          state.enemies = (state.enemies || []).map(e => 
            e.id === t?.id ? { ...e, isStunned: eff.status === 'stunned' } : e
          );
        } else {
          state.player.isStunned = eff.status === 'stunned';
        }
...
${targetActor.type === 'enemy' ? 'Enemy' : 'Player'} consumed by Lava.`.trim());
      ...
      if (!res.moved && res.reason === 'wall') {
        ...
        if (targetActor.type === 'enemy') state.enemies = (state.enemies || []).map(e => e.id === targetActor.id ? { ...e, isStunned: true } : e);
        else state.player.isStunned = true;
